...
function journalCard(item,index=0){
  ...
  const media=image?`<div class="journal-news-media"><img src="${esc(image)}" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer"></div>`:`<div class="journal-news-media journal-news-fallback"><span>${String(index+1).padStart(2,'0')}</span><b>${esc(normalizeJournalGroup(category))}</b></div>`;
...
applySiteContent(SITE_DATA);
document.addEventListener('DOMContentLoaded',async()=>{await loadSiteData();await renderJournalPreview();bindReveal();bindVideo();});
