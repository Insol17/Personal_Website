...
async function init(){
  try{const res=await fetch('../assets/data/journal.json',{cache:'no-store'});if(!res.ok)throw new Error();const data=await res.json();posts=data.posts||data;render();}
  catch{posts=FALLBACK;render();}
  document.querySelectorAll('[data-filter]').forEach(btn=>btn.addEventListener('click',()=>{document.querySelectorAll('[data-filter]').forEach(b=>b.classList.toggle('is-active',b===btn));render(btn.dataset.filter)}));
}
init();
