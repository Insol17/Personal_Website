...
if(backButton){
  backButton.addEventListener('click',async()=>{
    ...
    const payload={slug:DETAIL_SLUG,image,scrollY:Number.isFinite(origin?.scrollY)?origin.scrollY:0,railIndex:Number.isInteger(origin?.railIndex)?origin.railIndex:null,railX:Number.isFinite(origin?.railX)?origin.railX:null,returnUrl,at:Date.now()};
