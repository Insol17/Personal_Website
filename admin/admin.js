...
async function importBackup(file){
  ...
  }catch{showToast('올바른 포트폴리오 콘텐츠 백업이 아닙니다.');}
}
...
function migrateProjectHTMLToV33(html,project){
  ...
  if(!next.includes('project-detail-v42.css')) next=next.replace(/(<link[^>]+project-detail-v41\.css[^>]*>)/i,'$1<link rel="stylesheet" href="project-detail-v42.css">');
  if(!next.includes('project-detail-v42.js')) next=next.replace(/(<script[^>]+project-detail-v41\.js[^>]*><\/script>)/i,'$1<script defer src="project-detail-v42.js"></script>');
  if(!next.includes('project-detail-v43.css')) next=next.replace(/(<link[^>]+project-detail-v42\.css[^>]*>)/i,'$1<link rel="stylesheet" href="project-detail-v43.css">');
  if(!next.includes('project-detail-v43.js')) next=next.replace(/(<script[^>]+project-detail-v42\.js[^>]*><\/script>)/i,'$1<script defer src="project-detail-v43.js"></script>');
  if(!next.includes('project-detail-v44.css')) next=next.replace(/(<link[^>]+project-detail-v43\.css[^>]*>)/i,'$1<link rel="stylesheet" href="project-detail-v44.css">');
  if(!next.includes('project-detail-v44.js')) next=next.replace(/(<script[^>]+project-detail-v43\.js[^>]*><\/script>)/i,'$1<script defer src="project-detail-v44.js"></script>');
  if(!next.includes('project-detail-v45.css')) next=next.replace(/(<link[^>]+project-detail-v44\.css[^>]*>)/i,'$1<link rel="stylesheet" href="project-detail-v45.css">');
  if(!next.includes('project-detail-v45.js')) next=next.replace(/(<script[^>]+project-detail-v44\.js[^>]*><\/script>)/i,'$1<script defer src="project-detail-v45.js"></script>');
  if(!next.includes('project-detail-v48.css')) next=next.replace(/(<link[^>]+project-detail-v45\.css[^>]*>)/i,'$1<link rel="stylesheet" href="project-detail-v48.css">');
  if(!next.includes('project-detail-v48.js')) next=next.replace(/(<script[^>]+project-detail-v45\.js[^>]*><\/script>)/i,'$1<script defer src="project-detail-v48.js"></script>');
  // v47 folder structure: keep legacy-content migration, then normalize runtime paths.
  next=next.replaceAll('../v41.css','../styles/runtime/v41.css').replaceAll('../v41.js','../scripts/runtime/v41.js').replaceAll('../global-transition-v41.js','../scripts/global-transition.js');
  if(!next.includes('../styles/v48.css')) next=next.replace(/(<link[^>]+\.\.\/styles\/runtime\/v41\.css[^>]*>)/i,'$1<link rel="stylesheet" href="../styles/v48.css">');
  for(const v of [41,42,43,44,45,48]){
    next=next.replace(new RegExp('([\"\'])project-detail-v'+v+'\\.css','g'),'$1runtime/project-detail-v'+v+'.css');
    next=next.replace(new RegExp('([\"\'])project-detail-v'+v+'\\.js','g'),'$1runtime/project-detail-v'+v+'.js');
  }
  ...
  try{
    ...
    for(const p of site.projects){
      ...
      if(!existing){
        ...
        if(html!==before){await githubPut(owner,repo,htmlPath,branch,token,textBase64(html),`Sync ${p.title} project shell`);write(`✓ ${htmlPath} (current shell)`)}
      }
    }
