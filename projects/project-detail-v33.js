...
if(backButton){
  backButton.addEventListener('click',async()=>{
    ...
    const returnUrl=(origin?.url&&origin?.path)?origin.url:fallback;
    const image=detailHeroImage?.currentSrc||detailHeroImage?.src||'';
    const payload={slug:DETAIL_SLUG,image,scrollY:Number.isFinite(origin?.scrollY)?origin.scrollY:0,railIndex:Number.isInteger(origin?.railIndex)?origin.railIndex:null,returnUrl,at:Date.now()};
...
async function revealDetailPage(){
  ...
  const origin=detailReadJSON('portfolioTransitionOriginState');
  const image=detailHeroImage?.currentSrc||detailHeroImage?.src||'';
  if(origin?.path&&image)detailWriteJSON('portfolioLastDetail',{slug:DETAIL_SLUG,image,origin,at:Date.now()});
